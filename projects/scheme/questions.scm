...
(define (enumerate s)
  ; BEGIN PROBLEM 15
  (define (helper x t) (if (null? t) nil
                         (cons (list x (car t)) (helper (+ x 1) (cdr t)))))
  (helper 0 s)
  )
  ; END PROBLEM 15
...
(define (zip pairs)
  (if (null? pairs) (cons nil (cons nil nil))
    (begin (define tmp (zip (cdr pairs))) 
           (cons (cons (caar pairs) (car tmp)) 
                 (cons
                   (cons (cadr (car pairs)) (cadr tmp))
                   nil
                   )
           )
           )
