...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def in_paragraph(s):
        remove_s = remove_punctuation(s)
        lower_s = lower(remove_s)
        tokens = split(lower_s)
        for t in topic:
            if t in tokens:
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    maxlen = max(len(typed_words), len(source_words))
    ...
    for i in range(minlen):
        if typed_words[i] == source_words[i]:
            equal += 1
    return equal / len(typed_words) * 100
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    pos = min(range(len(word_list)), key=lambda i: diff_function(typed_word, word_list[i], limit))
...
def feline_fixes(typed, source, limit):
    ...
    # def diff(s1, s2, change_number):
    #     if len(s1) == 0 or len(s2) == 0:
    #         next_change = change_number + len(s1) + len(s2)
    #         return limit+1 if next_change > limit else next_change
    #     if s1[0] != s2[0]:
    #         change_number += 1
    #     if change_number > limit:
    #         return limit+1
    #     return diff(s1[1:], s2[1:], change_number)
    if limit < 0:
        return 0
    if len(typed) == 0 or len(source) == 0:
        return len(typed) + len(source)
    return feline_fixes(typed[1:], source[1:], limit) if typed[0] == source[0] else 1+feline_fixes(typed[1:], source[1:], limit-1)
        
    return diff(typed, source, 0)
    # END PROBLEM 6
...
def final_diff(typed, source, limit):
    ...
    if limit < 0:  # Fill in the condition
        ...
        if typed[0] == source[0]:
            ...
            substitute = minimum_mewtations(typed[2:], source[2:], limit-1) + 1
        else:
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(words)):
            each.append(times_per_player[i][j+1]-times_per_player[i][j])
        times.append(each)
...
def fastest_words(match):
    ...
    for i in word_indices:
        fast_player = min(player_indices, key=lambda player: time(match, player, i))
        result[fast_player].append(get_word(match, i))
    return result
