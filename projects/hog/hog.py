...
def perfect_square(score):
    """Judge whether a score is a prefect square."""
    d = 1
    while d * d < score:
        d += 1
    if d * d == score:
...
def next_perfect_square(score):
    """Obtain the next prefect square larger than score."""
    d = 1
    while d * d <= score:
        d += 1
    return d * d
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        ...
        while opponent_score < goal:
            if num_rolls != strategy(score, opponent_score):
                return False
            opponent_score += 1
        score += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    i = 1
    while i <= 10:
        ave = make_averaged(roll_dice, total_samples)(i, dice)
        if ave > max_ave:
            max_ave = ave
            max_i = i
        i += 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    # global six_ave 
    # = make_averaged(roll_dice)(6)
    if square_update(0, score, opponent_score) >= 100:
        return 0
    elif square_update(0, score, opponent_score) - score >= 9:
        return 0
    else:
