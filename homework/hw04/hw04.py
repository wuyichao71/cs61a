...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    return balanced(end(left_arm)) and balanced(end(right_arm)) and length(left_arm)*total_weight(end(left_arm)) == length(right_arm)*total_weight(end(right_arm))
...
def has_path(t, word):
    ...
    one_true = False
    for b in branches(t):
        one_true = one_true or has_path(b, word[1:])
    return one_true and label(t) == word[0]
