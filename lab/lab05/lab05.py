...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_tmp = make_city('', lat, lon)
    return get_name(city_b) if distance(city_b, city_tmp) < distance(city_a, city_tmp) else get_name(city_a)
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
      return tree(label(t), branches=[tree(b) for b in leaves])
...
def add_trees(t1, t2):
    ...
    new_branches = [add_trees(b1, b2) for b1, b2 in zip(bs1, bs2)]
    new_branches += (bs1[len(new_branches):] + bs2[len(new_branches):])
